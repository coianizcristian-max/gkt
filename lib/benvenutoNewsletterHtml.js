 ...
 * @param variante  'demo' (default): iscritto dal popup della demo, l'ha gia'
 *                  vista -> pulsante principale "Crea il tuo account".
 *                  'home': iscritto dal form della home, la demo non l'ha vista
 *                  -> pulsante principale "Guarda la stagione demo".
 *                  Grafica e resto del contenuto identici.
 */
export function benvenutoNewsletterHtml({ m, links, siteUrl, variante = 'demo' }) {
  const logo = `${siteUrl}/gk_circle_white.png`
  const hero = `${siteUrl}/email/benvenuto-hero.jpg`
  const home = variante === 'home'
  const invito = home ? m('nlBvInvitoHome') : m('nlBvInvito')
  const primario = home
    ? { href: links.demo, testo: m('nlBvDemoBottone') }
    : { href: links.registrati, testo: m('nlBvBottone') }
  const secondario = home
    ? { href: links.registrati, testo: m('nlBvRegistrati') }
    : { href: links.demo, testo: m('nlBvDemo') }
  const piede = home ? m('nlBvFooterHome') : m('nlBvFooter')
      ...
      <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:580px;background:#ffffff;border-radius:14px;overflow:hidden;font-family:'Segoe UI',Arial,sans-serif;box-shadow:0 4px 24px rgba(0,0,0,0.10);">
          ...
          <td style="padding:4px 32px 30px;">
            <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:#f2f8fc;border:1px solid #d9e9f5;border-radius:12px;">
              <tr><td align="center" style="padding:24px 22px;">
                <p style="margin:0 0 18px;font-size:15px;line-height:1.6;color:#2a3b47;">${invito}</p>
                <a href="${primario.href}" target="_blank" style="display:inline-block;background:#0a7ec2;color:#ffffff;text-decoration:none;padding:14px 30px;border-radius:9px;font-weight:700;font-size:15px;">${primario.testo}</a>
                <div style="margin-top:14px;font-size:14px;"><a href="${secondario.href}" target="_blank" style="color:#0a7ec2;">${secondario.testo} &rarr;</a></div>
              </td></tr>
            </table>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#f6f8fa;border-top:1px solid #e8f0f8;padding:18px 32px;text-align:center;">
            <p style="margin:0 0 6px;font-size:12px;color:#8899a8;line-height:1.6;">${piede}</p>
