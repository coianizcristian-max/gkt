...
// e la prova viene ritentata al caricamento successivo.
// ────────────────────────────────────────────────────────────────────────────
export async function assicuraProva(supabaseUtente, user, profilo) {
  ...
  try {
    ...
    // PRIMA si crea la riga, POI si segna il flag. Prima era al contrario e
    // l'insert (un upsert su un vincolo che nel DB non esisteva) falliva in
    // silenzio: flag messo, prova mai creata, e mai più ritentata.
    if (giorni > 0) {
      const { data: esiste, error: e1 } = await admin.from('abbonamenti')
        .select('id').eq('allenatore_id', user.id).limit(1)
      if (e1) throw new Error('lettura abbonamenti: ' + e1.message)
      // Se esiste già una riga (pagato, manuale...) NON la tocchiamo.
      if (!esiste?.length) {
        const scadenza = new Date(Date.now() + giorni * 24 * 60 * 60 * 1000).toISOString()
        const { error: e2 } = await admin.from('abbonamenti')
          .insert({ allenatore_id: user.id, piano: 'prova', stato: 'prova', scadenza })
        // 23505 = creata un attimo fa da un'altra richiesta: va bene così
        if (e2 && e2.code !== '23505') throw new Error('insert prova: ' + e2.message)
      }
    }

    // Flag "una volta sola": messo solo quando la prova è davvero a posto;
    // se qualcosa è fallito sopra, al prossimo caricamento si ritenta.
    const { error: e3 } = await admin.from('profili').update({ prova_creata: true }).eq('id', user.id)
    if (e3) throw new Error('flag prova_creata: ' + e3.message)
