...
export default async function SupervisioneEsercizi({ params }) {
  ...
  const gruppi = {}
  lista.forEach(e => {
    const k = e.tipologia ?? t('altroTipo')
  ...
  return (
    <>
      <div className="topbar">
        ...
        <h1>{t('navEsercizi')} ({lista.length})</h1>
      </div>
      <div className="content">
        {lista.length === 0 && <div className="empty">{t('nessunEsercizioLib')}</div>}
              ...
              <div key={e.id} className="lista-riga" style={{ alignItems: 'flex-start', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--ink-soft)', marginTop: 4, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    {e.difficolta && <span>{t('difficolta')}: {e.difficolta}</span>}
