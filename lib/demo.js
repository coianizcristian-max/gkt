...
// Segna che la sessione corrente e' quella dell'utente vetrina entrato da /d.
// Serve solo a decidere dove mandarlo quando esce dalla demo: un ospite non ha
// dati propri, quindi va riportato al sito pubblico e non in una app vuota.
export const COOKIE_OSPITE = 'gk_ospite'
// Il visitatore e' arrivato da un link della newsletter: e' gia' iscritto,
// quindi non gli si propone di iscriversi.
export const COOKIE_DA_NEWSLETTER = 'gk_da_nl'
...
/** True se la sessione corrente e' quella dell'utente vetrina (ingresso da /d). */
export const inOspite = cache(async () => {
  try {
    const store = await cookies()
    return store.get(COOKIE_OSPITE)?.value === '1'
  } catch {
    return false
  }
})

/** True se il visitatore e' arrivato da un link della newsletter. */
export const daNewsletter = cache(async () => {
  try {
    const store = await cookies()
    return store.get(COOKIE_DA_NEWSLETTER)?.value === '1'
  } catch {
    return false
  }
})
...
/**
 * Owner da usare per filtrare i dati: in demo e' l'account demo, altrimenti
 * l'owner reale dell'utente. Sostituisce getOwnerId nelle pagine che leggono
 * dati di stagione: filtrare i dati demo con l'id dell'utente reale non
 * restituisce nulla (o peggio, restituisce i suoi).
 */
export async function ownerDati(supabase, userId) {
  const { getOwnerId } = await import('@/lib/tenant')
  if (await inDemo()) {
    const cfg = await getDemoConfig()
    if (cfg.attiva && cfg.ownerId) {
      const { data: profilo } = await supabase
        .from('profili').select('ruolo').eq('id', userId).maybeSingle()
      if (profilo?.ruolo === 'allenatore' && userId !== cfg.ownerId) return cfg.ownerId
    }
  }
  return getOwnerId(supabase, userId)
}
