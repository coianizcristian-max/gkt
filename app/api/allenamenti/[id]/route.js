...
export async function DELETE(request, { params }) {
  try {
    ...
    if (!allenamento) return NextResponse.json({ error: tApi(request, 'Allenamento non trovato.') }, { status: 404 })

    const { data: stagione } = await supabase
      .from('stagioni').select('id').eq('id', allenamento.stagione_id).eq('owner_id', ownerId).maybeSingle()
    if (!stagione) return NextResponse.json({ error: tApi(request, 'Non autorizzato.') }, { status: 403 })
