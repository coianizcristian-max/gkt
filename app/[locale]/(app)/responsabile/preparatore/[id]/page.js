...
export default async function SupervisionePanoramica({ params }) {
  ...
  const sezioni = [
    { href: `${base}/portieri`, label: t('navPortieri'), emoji: '🧤', count: portieri.length },
    { href: `${base}/calendario`, label: t('navCalendario'), emoji: '📅', count: ultimiAllenamenti.length > 0 ? t('recenti', { n: ultimiAllenamenti.length }) : '—' },
    { href: `${base}/partite`, label: t('navPartite'), emoji: '⚽', count: ultimaPartita ? t('ultima', { avv: ultimaPartita.avversario }) : '—' },
    { href: `${base}/statistiche`, label: t('navStatistiche'), emoji: '📊', count: '' },
    { href: `${base}/esercizi`, label: t('navEsercizi'), emoji: '📋', count: '' },
  ]
  ...
  return (
    <>
      <div className="topbar">
        ...
        <h1>{profilo?.nome_completo ?? t('preparatoreFallback')}</h1>
      </div>
      <div className="content">
        ...
        <div className="scheda" style={{ display: 'flex', gap: 14, alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {stagioneAttiva && <div style={{ fontSize: 12, color: 'var(--verde)', marginTop: 4 }}>{t('stagioneAttiva', { nome: stagioneAttiva.nome })}</div>}
          </div>
        </div>
