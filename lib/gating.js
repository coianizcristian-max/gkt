...
export const FUNZIONALITA = {
  ...
  esercizi_libreria:       { label: 'Libreria esercizi personale',                    defaultFree: true },
  ...
  inviti_staff:            { label: 'Invito staff/preparatore (supervisione)',        defaultFree: true },
  profilo_ricerca:         { label: 'Profilo allenatore nella ricerca pubblica',     defaultFree: false },
  export_dati:             { label: 'Export dati (CSV portieri/valutazioni/partite)', defaultFree: true },
  report_pdf_stagione:     { label: 'Report PDF stagione',                            defaultFree: true },
  report_pdf_statistiche:  { label: 'Report PDF statistiche squadra (per mese/categoria)', defaultFree: false },
}

/**
 * Lato server: restituisce { tuttoFree, config }
 * config = { chiave: boolean (true = free/sbloccato) }
 * cache(): questa configurazione è identica per tutta la richiesta, gira una
 * sola volta invece di essere riletta a ogni pagina che la usa.
 */
