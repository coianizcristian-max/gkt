...
export async function generateMetadata({ params }) {
  ...
  const t = await getTranslations({ locale, namespace: 'allenatorePubblico' })
  const profilo = await getProfiloPubblico(id)

  if (!profilo) return { title: t('metaNonTrovato') }

  const nome = profilo.nome_completo || t('allenatoreFallback')
  const desc = profilo.bio
    ? profilo.bio.slice(0, 155)
    : (profilo.citta ? t('metaDescCitta', { citta: profilo.citta }) : t('metaDescNoCitta'))
...
export default async function ProfiloAllenatorePublicPage({ params }) {
  ...
  const t = await getTranslations('allenatorePubblico')
  const profilo = await getProfiloPubblico(id)
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '40px 20px' }}>
      ...
      <Link href="/cerca-allenatori" className="link-inline" style={{ fontSize: 13 }}>{t('tornaRicerca')}</Link>
      ...
      {profilo.bio && (
        <div className="scheda" style={{ marginBottom: 16 }}>
          <h2 style={{ margin: '0 0 10px', fontSize: 16 }}>{t('bio')}</h2>
      ...
      {esperienze.length > 0 && (
        <div className="scheda" style={{ marginBottom: 16 }}>
          <h2 style={{ margin: '0 0 10px', fontSize: 16 }}>{t('esperienze')}</h2>
      ...
      {certificati.length > 0 && (
        <div className="scheda" style={{ marginBottom: 16 }}>
          <h2 style={{ margin: '0 0 10px', fontSize: 16 }}>{t('certificati')}</h2>
          <ul style={{ margin: 0, paddingLeft: '1.2rem', lineHeight: 1.8, color: 'var(--ink-soft)' }}>
      ...
      <div className="scheda" style={{ marginBottom: 0 }}>
        <h2 style={{ margin: '0 0 10px', fontSize: 16 }}>{t('contattaTitolo')}</h2>
        <div className="paywall-banner">
          <div className="paywall-icon">🔒</div>
          <div className="paywall-text">
            <b>{t('contattiRiservati')}</b>
            <p>{t.rich('contattoTesto', { importo: importoFee, b: (ch) => <strong>{ch}</strong> })}</p>
          </div>
