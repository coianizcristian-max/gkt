...
export async function consumaInvito(token, user) {
  ...
  if (invito.tipo === 'portiere' || invito.tipo === 'collaboratore') {
    ...
    if ((stagioniPossedute ?? 0) > 0) {
      return {
        error: "Questo account possiede già delle stagioni (è un allenatore) e non può essere collegato come portiere o collaboratore. Usa l'invito con un account nuovo.",
        status: 409,
      }
    }
  }
