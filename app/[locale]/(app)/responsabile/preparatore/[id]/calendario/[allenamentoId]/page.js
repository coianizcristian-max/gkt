    ...
    <>
      <div className="topbar">
        <div className="eyebrow">
          <Link href={`${basePath}/calendario`}>{t('backCalendario')}</Link>
        </div>
        <h1>
          {all.squadra?.nome}
      ...
      <div className="content">
        ...
        <div className="scheda" style={{ marginBottom: 16 }}>
          ...
          {esercizi.length === 0
            ? <div className="empty">{t('nessunEsercizioPianificato')}</div>
        ...
        {!all.nessuna_valutazione && portieri.length > 0 && (
          <div className="scheda" style={{ marginBottom: 16 }}>
            <h3 style={{ marginTop: 0 }}>{t('valutazioniPortieri')}</h3>
            {portieri.map(p => {
              const v = valMap[p.id]
              ...
              return (
                <div key={p.id} className="lista-riga" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 6 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%' }}>
                    ...
                    {v?.presente === false && <span style={{ fontSize: 12, color: 'var(--rosso)' }}>{t('assente')}</span>}
                    {v?.presente === true && v?.voto != null && (
                      <span style={{ fontWeight: 700, fontSize: 16 }}>⭐ {v.voto}</span>
                    )}
                    {!v && <span style={{ fontSize: 12, color: 'var(--ink-soft)' }}>{t('nonValutato')}</span>}
                  </div>
        ...
        {feedback.length > 0 && (
          <div className="scheda" style={{ marginBottom: 16 }}>
            <h3 style={{ marginTop: 0 }}>{t('feedbackPortieri')}</h3>
            {feedback.map((r, i) => (
              <div key={i} className="lista-riga" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 4 }}>
                ...
                {r.voto_portiere != null && <div style={{ fontSize: 13 }}>{t('autovalutazione')}: <b>{r.voto_portiere}</b></div>}
              </div>
            ))}
          </div>
        )}
