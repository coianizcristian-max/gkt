...
export const getStagioneAttiva = cache(async function getStagioneAttiva(supabase, userId) {
  ...
  const { data: profiloUtente } = await supabase
    .from('profili').select('ruolo, portiere_id, stagione_corrente_id').eq('id', userId).maybeSingle()

  // ── PORTIERE ──────────────────────────────────────────────────────
  // La stagione del portiere è quella in cui è ISCRITTO, NON "la più recente"
  // dell'allenatore. Un coach può avere più stagioni attive insieme (più
  // società): scegliere la più recente per created_at dirotterebbe il portiere
  // in una stagione che non è la sua. Il portiere non ha il selettore in alto,
  // quindi il suo puntatore personale è irrilevante: risolviamo sempre
  // dall'iscrizione e riallineiamo il puntatore.
  if (profiloUtente?.ruolo === 'portiere' && profiloUtente.portiere_id) {
    const { data: iscr } = await supabase
      .from('iscrizioni').select('stagione_id').eq('portiere_id', profiloUtente.portiere_id)
    const ids = (iscr ?? []).map((r) => r.stagione_id)
    if (ids.length) {
      const { data: stag } = await supabase
        .from('stagioni').select('*')
        .eq('owner_id', ownerId).eq('attiva', true).in('id', ids)
        .order('created_at', { ascending: false }).limit(1).maybeSingle()
      if (stag) {
        if (profiloUtente.stagione_corrente_id !== stag.id) {
          await supabase.from('profili').update({ stagione_corrente_id: stag.id }).eq('id', userId)
        }
        return { stagione: stag, ownerId }
      }
    }
    // Non iscritto ad alcuna stagione attiva del suo allenatore: nessuna stagione.
    // Meglio uno stato vuoto ("lo staff ti iscriverà") che dirottarlo su una
    // stagione non sua.
    return { stagione: null, ownerId }
  }
