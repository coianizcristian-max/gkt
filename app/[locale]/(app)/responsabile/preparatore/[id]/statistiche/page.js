...
export default async function SupervisioneStatistiche({ params }) {
  ...
  if (!stagione) {
    return (
      <>
        <div className="topbar"><div className="eyebrow">—</div><h1>{t('navStatistiche')}</h1></div>
    ...
    <>
      ...
      <div className="content">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: 10, marginBottom: 16 }}>
          {[
            { label: t('statAllenamenti'), val: totAllenamenti, emoji: '📅' },
            { label: t('statPartite'), val: totPartite, emoji: '⚽' },
            { label: t('statGolSubiti'), val: golSubiti, emoji: '🥅' },
            { label: t('statMediaGol'), val: mediaGol, emoji: '📊' },
          ].map(k => (
        ...
        <div className="scheda">
          <h3 style={{ marginTop: 0 }}>{t('navPortieri')}</h3>
            ...
            <div key={p.id} className="lista-riga" style={{ alignItems: 'center', gap: 12 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--ink-soft)' }}>
                  {p.presenze}/{p.disponibili} {t('allenamentiLbl')} ({p.percPresenza}%){p.persi > 0 ? ` \u00b7 \ud83e\ude79 ${p.persi}` : ''}
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontWeight: 700, fontSize: 16 }}>{p.mediaVoto}</div>
                <div style={{ fontSize: 11, color: 'var(--ink-soft)' }}>{t('mediaLbl')}</div>
              </div>
            </div>
          ))}
        </div>
