...
export default function TemplateEsercizi({ templateId, libreriaMia = [], libreriaPubblica = [], eserciziResponsabile = [], selezionatiIniziali, selezionatiEsercizi = [], attributiDisponibili = [] }) {
  ...
  async function salva() {
    ...
    const supabase = createClient()
    try {
      await supabase.from('template_allenamento_esercizi').delete().eq('template_id', templateId)
      const rows = ordine.filter((id) => sel.has(id)).map((eid, i) => ({ template_id: templateId, esercizio_id: eid, ordine: i }))
      if (rows.length) {
        const { error: iErr } = await supabase.from('template_allenamento_esercizi').insert(rows)
        if (iErr) throw iErr
      }
