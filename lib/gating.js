...
// Lettura dello stato abbonamento con il client di servizio (solo server):
// l'esito non dipende dalle policy RLS (es. lo staff deve poter "vedere"
// l'abbonamento del titolare). Se la chiave manca, usa il client passato.
function clientLettura(supabase) {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY
  if (typeof window === 'undefined' && url && key) return createAdminClient(url, key)
  return supabase
}
...
export const hasAbbonamento = cache(async function hasAbbonamento(supabase, userId) {
  ...
  const db = clientLettura(supabase)
  ...
  // coupon: il proprio, e per lo staff anche quello del titolare
  const { data: coupon } = await db
    .from('coupon_utilizzi')
    .select('scade_il')
    .in('utente_id', targetId === userId ? [userId] : [userId, targetId])
