...
export default async function AllenamentoPage({ params }) {
    ...
    return (
      <>
        <div className="topbar">
          <div className="eyebrow"><Link href="/calendario">{tc('titolo')}</Link></div>
  ...
  async function caricaEserciziResponsabile(supervisoreId) {
    ...
    try {
      ...
      const nomeResp = profResp?.nome_completo ?? t('responsabileFallback')
      ...
      <div className="content">
        {assentiAnnunciati.length > 0 && (
          <div className="ok-msg" style={{ marginBottom: 16, background: '#fff8e6', border: '1px solid #f0d98a' }}>
            📅 <b>{t('assentiAnnunciati')}</b> {assentiAnnunciati}
          </div>
        )}
        <AllenamentoTabs
          dettaglio={
          ...
          valutazioni={
            !canValutare ? (
              <PaywallBanner label={t('paywallValutazioni')} wrap>
          ...
          esercizi={
            <>
              ...
              /> : <PaywallBanner label={t('paywallEsercizi')} />}
            </>
          }
          feedback={
            canFeedback && feedback.length > 0 ? (
              <>
                <h2 className="sezione-titolo" style={{ marginTop: 24 }}>{t('feedbackPortieri')}</h2>
