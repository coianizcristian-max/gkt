...
export default async function PercorsoCrescitaPage({ params }) {
  ...
  const t = await getTranslations('percorso')
  ...
  if (stagione) {
    ...
    for (const v of valutazioni ?? []) {
      ...
      if (!allen) continue
      if (voto >= 8) eventi.push({ tipo: 'voto_alto', data: allen.data, titolo: t('eventoVotoAlto', { voto }), dettaglio: allen.squadra?.nome })
      if (voto <= 4) eventi.push({ tipo: 'voto_basso', data: allen.data, titolo: t('eventoVotoBasso', { voto }), dettaglio: allen.squadra?.nome })
    }
    ...
    for (const v of valPartite ?? []) {
      ...
      eventi.push({
        ...
        data: p.data,
        titolo: cs ? t('eventoCleanSheet', { avv: p.avversario || '—' }) : t('eventoPartita', { avv: p.avversario || '—' }) + (v.voto ? t('votoSuffix', { voto: v.voto }) : ''),
        dettaglio: p.tipo,
      })
    }
  }
