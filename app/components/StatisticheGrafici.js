...
function Grafico({ titolo, punti, colore = '#0a7ec2', tipo = 'line', min = 4, max = 10, empty }) {
  const t = useTranslations('statisticheGrafici')
  const locale = useLocale()
  const nl = NUM_LOCALE[locale] || 'it-IT'
  const [espanso, setEspanso] = useState(false)
  const emptyMsg = empty ?? t('nessunDato')
  const mLabel = (d) => {
    const p = String(d).slice(0, 7).split('-')
    if (p.length < 2) return String(d)
    return new Date(2000, Number(p[1]) - 1, 1).toLocaleDateString(nl, { month: 'short' })
  }
  ...
  const innerW = espanso ? Math.max((punti.length - 1) * W_PUNTO_ESPANSO, 200) : (VIEW_W_COMPATTO - PAD.l - PAD.r)
  const totalW = innerW + PAD.l + PAD.r
  const totalH = H + PAD.t + PAD.b
  const px = (i) => punti.length > 1 ? PAD.l + (i / (punti.length - 1)) * innerW : PAD.l + innerW / 2
    ...
    <div className="scheda" style={{ marginBottom: 14 }}>
      ...
      <div style={{ overflowX: espanso ? 'auto' : 'visible', WebkitOverflowScrolling: 'touch' }}>
        <svg viewBox={`0 0 ${totalW} ${totalH}`} preserveAspectRatio="xMidYMid meet"
          style={{ width: espanso ? Math.max(totalW, 300) : '100%', minWidth: espanso ? Math.max(totalW, 300) : 'auto', height: 'auto', display: 'block' }}>
          {ticks.map((tk, i) => (
            <g key={i}>
              <line x1={PAD.l} y1={py(tk)} x2={totalW - PAD.r} y2={py(tk)} stroke="#e2e6e1" strokeWidth="1" />
              <text x={PAD.l - 4} y={py(tk) + 4} textAnchor="end" fontSize="9" fill="#4a5b68">{tk}</text>
            </g>
          ))}
          {tipo === 'bar'
            ? punti.map((p, i) => {
                ...
                return (
                  <g key={i}>
                    <rect x={px(i) - barW / 2} y={py(p.y)} width={barW} height={PAD.t + H - py(p.y)} fill={colore} fillOpacity="0.8" rx="2" />
                    {i % everyN === 0 && (
...
export default function StatisticheGrafici({ dati, nomPortiere }) {
  ...
  return (
    <div>
      <h2 className="sezione-titolo">{t('titoloSezione')}</h2>

      <Grafico titolo={t('g1')} punti={g1} colore="#0a7ec2" min={4} max={10} empty={t('g1empty')} />
      <Grafico titolo={t('g2')} punti={g2} colore="#0a7ec2" min={4} max={10} empty={t('g2empty')} />
      <Grafico titolo={t('g3')} punti={g3} colore="#7c3aed" min={4} max={10} empty={t('g3empty')} />
      <Grafico titolo={t('g4')} punti={g4} colore="#7c3aed" min={4} max={10} empty={t('g4empty')} />
      <Grafico titolo={t('g5')} punti={g5} colore="#c0392b" min={0} max={Math.max(...(g5.map((p) => p.y)), 5)} empty={t('g5empty')} />
      {g6.length > 0 && (
        <Grafico titolo={t('g6')} punti={g6} colore="#e8a72c" min={0} max={Math.max(...g6.map((p) => p.y), 5)} />
      )}
      {g7.length > 0 && (
        <Grafico titolo={t('g7', { nome: nomPortiere })} punti={g7.map((m) => ({ x: m.mese + '-01', y: m.io }))}
