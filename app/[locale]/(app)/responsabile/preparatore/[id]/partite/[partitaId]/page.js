  ...
  return (
    <>
      <div className="topbar">
        <div className="eyebrow"><Link href={`${basePath}/partite`}>{t('backPartite')}</Link></div>
        <h1>
      ...
      <div className="content">
        ...
        <div className="scheda" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            ...
            <div>
              {partita.tipo && <div style={{ fontSize: 13 }}>{t('tipo')}: {partita.tipo}</div>}
              {partita.gol_subiti === 0 && partita.gol_fatti != null && (
                <div style={{ fontSize: 13, color: 'var(--verde)', fontWeight: 600 }}>{t('cleanSheet')}</div>
              )}
            </div>
          </div>
          {partita.note && (
        ...
        {presenti.length > 0 && (
          <div className="scheda" style={{ marginBottom: 16 }}>
            <h3 style={{ marginTop: 0 }}>{t('valutazioniPortieri')} ({presenti.length})</h3>
            {presenti.map(v => (
        ...
        {assenti.length > 0 && (
          <div className="scheda">
            <h3 style={{ marginTop: 0 }}>{t('assenti')} ({assenti.length})</h3>
            {assenti.map(v => (
