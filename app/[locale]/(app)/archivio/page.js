...
export default async function ArchivioPage({ searchParams }) {
  ...
  const t = await getTranslations('archivio')
  ...
  if (!stagioni || stagioni.length === 0) {
    return (
      <>
        <div className="topbar"><div className="eyebrow">{t('eyebrow')}</div><h1>{t('titolo')}</h1></div>
        <div className="content"><div className="empty">{t('vuotaStagioni')}</div></div>
      </>
    )
  }
  ...
  const esitoLabel = { V: t('esitoV'), P: t('esitoP'), X: t('esitoX') }

  return (
    <>
      <div className="topbar topbar-row">
        <div><div className="eyebrow">{t('solaLettura')}</div><h1>{t('titolo')}</h1></div>
      </div>
      <div className="content">
        ...
        {canExport
          ...
          : <PaywallBanner chiave="export_dati" label={t('paywallLabel')} />}
        {!stagione.attiva && <p className="sub-intro">{t('avvisoArchiviata')}</p>}

        <h2 className="sezione-titolo">{t('statistiche')}</h2>
        {stats.length === 0 ? <div className="empty">{t('nessunPortiere')}</div> : (
          categorieOrd.map((cat) => {
            const lista = byCat[cat.id] ?? []
            if (lista.length === 0) return null
            return (
              <section key={cat.id}>
                <div className="squadra-head"><h2>{cat.nome}</h2><span className="conta">{t('contaPortieri', { count: lista.length })}</span></div>
                    ...
                    <div className="stat-card" key={s.p.id}>
                      ...
                      <div className="stat-rows">
                        <div className="stat-block">
                          <h4>{t('allenamenti')}</h4>
                          <div className="stat-line"><span>{t('presenze')}</span><b>{s.presenze}/{s.totAllen}</b></div>
        ...
        <h2 className="sezione-titolo">{t('partite')}</h2>
        {(part ?? []).length === 0 ? <div className="empty">{t('nessunaPartita')}</div> : (
            ...
            {(part ?? []).map((p) => {
              ...
              return (
                <div className="partita-row" key={p.id} style={{ cursor: 'default' }}>
                  ...
                  <span className="pr-match">{p.casa ? t('casa') : t('trasferta')} &middot; vs {p.avversario || '\u2014'}</span>
                  ...
                  <span className="pr-cs-cell">{cs && <span className="badge-cs">{t('cleanSheet')}</span>}</span>
                </div>
              )
            })}
          </div>
