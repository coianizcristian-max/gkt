...
export default function CouponBox({ onAttivato }) {
  const t = useTranslations('couponBox')
  ...
  async function attiva() {
    ...
    } catch (e) { setError(t('erroreRete')) }
    setLoading(false)
  }

  if (ok) return (
    <div className="ok-msg" style={{ textAlign: 'center', padding: 16 }}>
      {t.rich('attivato', { gg: ok.durata_gg, b: (ch) => <b>{ch}</b> })}
    </div>
  )

  return (
    <div className="scheda" style={{ marginTop: 16 }}>
      <h3 style={{ marginTop: 0, marginBottom: 8 }}>{t('titolo')}</h3>
      <p className="sub-intro" style={{ marginBottom: 12 }}>{t('intro')}</p>
