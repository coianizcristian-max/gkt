  ...
  return (
    <>
      <div className="topbar">
        <div className="eyebrow"><Link href={`${basePath}/portieri`}>{t('backPortieri')}</Link></div>
      ...
      <div className="content">
        ...
        <div className="scheda" style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {eta && <div style={{ fontSize: 13, color: 'var(--ink-soft)' }}>{eta} {t('anni')}</div>}
            {portiere.piede_preferito && <div style={{ fontSize: 13 }}>{t('piede')}: {portiere.piede_preferito}</div>}
            {portiere.altezza_cm && <div style={{ fontSize: 13 }}>{t('altezza')}: {portiere.altezza_cm} cm</div>}
            {portiere.peso_kg && <div style={{ fontSize: 13 }}>{t('peso')}: {portiere.peso_kg} kg</div>}
            {iscr && <div style={{ fontSize: 13, marginTop: 4 }}>
              {iscr.squadre?.nome} {iscr.numero_maglia ? `· #${iscr.numero_maglia}` : ''}
              {!iscr.attivo ? ' · ' + t('inattivo') : ''}
            </div>}
          </div>
        </div>
        ...
        {ultimiAllenamenti.length > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 16 }}>
            {[
              { label: t('presenzeLbl'), val: presenze, emoji: '✅' },
              { label: t('su') + ' ' + ultimiAllenamenti.length, val: (ultimiAllenamenti.length - presenze) + ' ' + t('assSuffix'), emoji: '📋' },
              { label: t('mediaVotoLbl'), val: mediaVoto ?? '—', emoji: '⭐' },
            ].map(k => (
        ...
        {ultimiAllenamenti.length > 0 && (
          <div className="scheda">
            <h3 style={{ marginTop: 0 }}>{t('ultimeValutazioni')}</h3>
              ...
              return (
                <div key={v.id} className="lista-riga" style={{ alignItems: 'center' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {!v.presente && <span style={{ fontSize: 12, color: 'var(--rosso)', marginLeft: 8 }}>{t('assente')}</span>}
                  ...
                  <Link href={`${basePath}/calendario/${v.allenamento_id}`} className="btn-mini">{t('apri')}</Link>
                </div>
              )
            })}
          </div>
